...
def extract_to_cache(artifact: Artifact, cache_dir: Path) -> int:
...
class ExtractionSubscriber:
    ...
    def handle(self, event: ScanEvent) -> None:
        ...
        if not Path(artifact.path).exists():
            return
        extract_to_cache(artifact, self.cache_dir)
...
def extract_all(
    ...
    """(Re)build the extraction cache for every indexed, on-disk artifact.

    Duplicates share a content id and therefore a single cache entry, so each
    distinct id is extracted once. Returns summary counters.
    """
    ...
    for row in rows:
        ...
        seen_ids.add(row["id"])
        try:
            links_extracted += extract_to_cache(_artifact_from_row(row), cache_path)
